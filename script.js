...
const ES_TOUCH = window.matchMedia('(hover: none), (pointer: coarse)').matches;
...
function inicializarCorazonesFlotantes() {
    if (REDUCE_MOTION || ES_TOUCH) return;

    const container = document.getElementById('corazones-container');
    const corazones = ['❤', '💖', '💕', '💗', '💝', '💝', '💘', '🤎', '💞', '💘'];

    // Crea un numero menor de corazones para mantener el efecto sin sobrecargar el rendimiento
    for (let i = 0; i < 10; i++) {
...
function inicializarLluviaCorazones() {
    ...
    // Genera una lluvia mas ligera para evitar lag
    for (let i = 0; i < 4; i++) {
        setTimeout(() => crearCorazonLluvia(container, corazones), i * 450);
    }

    setInterval(() => crearCorazonLluvia(container, corazones), 1800);
}
...
function crearExplosionCorazones() {
    ...
    const corazones = ['❤', '💖', '💕', '💗', '💝', '💘', '💓', '💞'];
    const cantidad = REDUCE_MOTION || ES_TOUCH ? 24 : 80;

    // Crea menos corazones en pantallas mas lentas para que no se sature el rendimiento
